...
//! SUM(LONG) kernel — `i64 -> f64` accumulation.
//!
...
///
/// Exposed publicly so the JNI layer can offer a forced-scalar entry point
/// (`Java_..._sumLongScalar`) for benchmarking three-way attribution
/// (Java vs Rust-scalar vs Rust-SIMD).
#[inline]
pub fn sum_i64_to_f64_scalar(values: &[i64]) -> f64 {
    let mut s0 = 0.0_f64;
    let mut s1 = 0.0_f64;
    let mut s2 = 0.0_f64;
    let mut s3 = 0.0_f64;
